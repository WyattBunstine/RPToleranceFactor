#!/usr/bin/env python3
"""Scatter plot for the 214 dataset using B-site vs A-site Shannon radii."""
...
DEFAULT_INPUT = Path("data/crystal_graph_data/crystal_graph_dataset_214_v1.csv")
DEFAULT_OUTPUT = Path("data/crystal_graph_data/crystal_graph_family_scatter_214_v1.png")
SIM_THRESHOLD = 0.5
COMMON_ANIONS = {"O", "F", "S", "Se", "Cl", "Br", "I", "N"}

# Column -> display label
FAMILY_COLS = {
    "similarity_Ruddlesden_Popper": "Ruddlesden-Popper",
    "similarity_spinel": "spinel",
    "similarity_olivine": "olivine",
    "similarity_arcanite": "arcanite",
}
...
FAMILY_COLORS = {
    "Ruddlesden-Popper": "tab:blue",
    "spinel": "tab:orange",
    "olivine": "tab:green",
    "arcanite": "tab:red",
    "Other structure": "tab:gray",
}
...
def _choose_family(row: pd.Series, threshold: float, active_cols: List[str]) -> Tuple[str, float]:
    ...
    if best_score < threshold:
        return "Other structure", best_score
...
def _amount_close(value: float | None, target: float, tol: float = 1e-3) -> bool:
    if value is None:
        return False
    return abs(float(value) - float(target)) <= tol


def _assign_a_b_x_species_214(
    formula: str,
    oxidation: Dict[str, float],
    shannon: Dict[str, float],
    coordination: Dict[str, float],
) -> Tuple[str | None, str | None, str | None]:
    # Default behavior if formula parsing fails.
    fallback = _assign_a_b_x_species(oxidation, shannon, coordination)
    ...
    cation_pool = cations if cations else [s for s in species if s not in anions]

    a_candidates = [s for s in cation_pool if _amount_close(amounts.get(s), 2.0)]
    b_candidates = [s for s in cation_pool if _amount_close(amounts.get(s), 1.0)]
    x_candidates = [s for s in anions if _amount_close(amounts.get(s), 4.0)]

    if not a_candidates or not b_candidates:
        return fallback

    # Use coordination/radius tie-breakers if multiple species match a stoichiometric slot.
    a_species = sorted(
        a_candidates,
        key=lambda s: (
            -float(coordination.get(s, 0.0)),
...
def build_plot(
    ...
    for row in df.itertuples(index=False):
        ...
        a_species, b_species, x_species = _assign_a_b_x_species_214(
            str(getattr(row, "formula", "")),
            ox_map,
            r_map,
            cn_map,
        )
    ...
    for family in FAMILY_ORDER:
        ...
        scatter_kwargs = {
            ...
            "s": marker_size if family != "Other structure" else marker_size * 1.2,
            "alpha": 0.58,
            "color": FAMILY_COLORS[family],
        }
        if family != "Other structure":
            scatter_kwargs["linewidths"] = 0.4
            scatter_kwargs["edgecolors"] = "none"
        else:
            scatter_kwargs["linewidths"] = 1.0
        ax.scatter(sub["b_rad"], sub["a_rad"], **scatter_kwargs)
    ...
    legend_handles = [
        Line2D(
            ...
            markeredgecolor=FAMILY_COLORS[f] if f == "Other structure" else "none",
            markersize=8,
            linestyle="None",
            label=f,
        )
    ...
    ax.legend(handles=line_handles, title="Goldschmidt Oxide Guide Lines", loc="lower right", fontsize=8)

    ax.set_xlabel("B-site Radius")
    ax.set_ylabel("A-site Radius")
    ax.set_title("214 Samples: Radius by Prototype Similarity")
    ax.grid(alpha=0.2)
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Plot rows from crystal_graph_dataset_214_v1.csv using "
            "x=B and y=A, with marker by closest family and configurable marker size."
        )
    )
    ...
    parser.add_argument(
        ...
        help="If max family similarity is below this, label as 'Other structure' (default: 0.5).",
    )
    parser.add_argument(
        "--marker-size",
        type=float,
