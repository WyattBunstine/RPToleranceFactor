#!/usr/bin/env python3
"""Scatter plot of B-site vs A-site radii with marker by closest prototype family."""
...
DEFAULT_OUTPUT = Path("data/crystal_graph_data/crystal_graph_family_scatter_v1.png")
SIM_THRESHOLD = 0.5
COMMON_ANIONS = {"O", "F", "S", "Se", "Cl", "Br"}
...
def _select_a_b_radii(
    ...
    # Fallback when oxidation info is missing/zero.
    ...
    # A-site tends to have larger CN / larger radius, B-site smaller.
    ranked = sorted(
        cations,
        key=lambda s: (
...
def build_plot(input_csv: Path, output_png: Path, similarity_threshold: float, dpi: int, show: bool) -> None:
    ...
    if plot_df.empty:
        raise RuntimeError("No rows available for plotting after A/B-site selection.")
    ...
    ax.set_title("Crystal Graph Dataset: A/B Radii by Prototype Similarity")
    ax.grid(alpha=0.2)
    fig.tight_layout()
    ...
    print(f"Skipped rows (unable to determine A/B): {skipped}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Plot B-site vs A-site Shannon radii from crystal_graph_dataset_v1.csv "
            "with marker by most similar family (Perovskite/ilmenite/calcite/pyroxene/none)."
        )
    )
