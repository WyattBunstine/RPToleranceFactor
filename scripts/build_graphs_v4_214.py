#!/usr/bin/env python3
"""
build_graphs_v4_214.py

Build v4 crystal graphs for all A2BX4 (2:1:4 stoichiometry) CIF files and
write them to data/crystal_graphs_v4/.  Skips files that already have a
corresponding output JSON unless --no-skip is given.

Targets the n=1 Ruddlesden-Popper / K2NiF4 stoichiometry (e.g., Sr2TiO4,
La2NiO4, La2CuO4, Sr2RuO4, …).  Counterpart to build_graphs_v4_113.py
which handles the ABX3 perovskite stoichiometry; the two together cover
the perovskite ↔ n=1-RP validation pair.

Usage:
    python build_graphs_v4_214.py
    python build_graphs_v4_214.py --cif-dir data/cifs --output-dir data/crystal_graphs_v4
    python build_graphs_v4_214.py --workers 8
    python build_graphs_v4_214.py --no-skip
    python build_graphs_v4_214.py --limit 50
"""
...
# ---------------------------------------------------------------------------
# 2:1:4 stoichiometry filter
# ---------------------------------------------------------------------------

def _is_214(stem: str) -> bool:
    """Return True if the CIF stem encodes an A2BX4 (2:1:4) composition."""
...
def build_graphs_v4(
    ...
    total = len(cif_paths)
    total_todo = len(todo)
    print(f"A2BX4 CIFs: {total} total, {skipped} cached, {total_todo} to build  ->  {output_dir}",
          flush=True)
    if total_todo == 0:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Build v4 crystal graphs for all A2BX4 (2:1:4) CIF files.",
    ...
    cif_paths = [c for c in all_cifs if _is_214(c.stem)]
    print(f"Found {len(cif_paths)} A2BX4 CIFs out of {len(all_cifs)} total in {cif_dir}")
