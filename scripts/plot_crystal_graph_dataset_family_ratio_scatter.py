#!/usr/bin/env python3
"""Scatter plot for all samples using B/X and A/X radius ratios."""
...
DEFAULT_OUTPUT = Path("data/crystal_graph_data/crystal_graph_family_ratio_scatter_v1.png")
SIM_THRESHOLD = 0.5
...
def _assign_a_b_x_species(
    ...
    # Fallback when oxidation cannot separate ions.
    ...
    # X: most anion-like first, then highest coordination.
    x_species = sorted(
        anions,
        key=lambda s: (
            float(oxidation.get(s, 0.0)),  # more negative first
    ...
    # B smaller/lower-coordination cation, A larger/higher-coordination cation.
...
def build_plot(
    ...
    # Label all plotted points with formula in a small font.
    ...
    ax.set_title("All Samples: Radius Ratios by Prototype Similarity")
    ax.grid(alpha=0.2)
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "x=B/X and y=A/X radius ratios, with marker by closest family."
        )
    )
