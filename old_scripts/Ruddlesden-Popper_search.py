...
def load_dataset(paper_dataset = False, artif_dataset = False):
    ...
    raw_df = pd.read_csv("../data/Old_datasets/RP_Datasets/cif_datasets/Ruddlesden-Popper_expanded_data_test.csv")
    print(len(raw_df))
    df = pandas.DataFrame(
        columns=["struct", "n", "A", "B", "X", "Aox", "Box", "Cox", "RaIX", "RaXII", "Rb", "Rc", "exp", "EAH",
                 "isperov", "SG"])
    if paper_dataset:
        ...
        df = pandas.DataFrame(
            columns=["struct", "n", "A", "B", "X", "RaIX", "Rb", "Rc", "RaXII", "Aox", "Box", "Cox", "EAH", "exp",
                     "isperov", "SG"])
    if artif_dataset:
        artif_df = pd.read_csv("../data/Old_datasets/RP_Datasets/cif_datasets/Ruddlesden-Popper_artificial_data.csv")
        print(artif_df)
        print(raw_df)
